...
% Mateo: cambié dc por CC y ca por CA
...
% Mateo: formas de onda de los inversores prácticos?? Pondría que los circuitos más simples de inversores generan una onda de salida cuadrada y que eso ya basta para algunas aplicaciones. Aunque esto venga de la referencia, me suena muy raro eso de prácticos.
...
% Mateo: Dice "para aplicaciones de baja y mediana frecuencia", comparado a "aplicaciones de alta potencia". Creo que está mal, sería baja y mediana potencia. 
...
% Chip no me gusta mucho, se podría poner CI (de circuito integrado). Para eso habría que aclararlo en el párrafo an terior (ej: El Circuito Integrado (CI) 555...)
...
% Mateo: hay q agregarle a todos los circuitos los puntos en todos los nodos.
...
% Mateo: Cambié IC por CI, con más razon hay q aplicar lo que dije anteriormente en el comentario de la línea 18
...
\snsubsubsection{Descripción de funcionamiento de un inversor de medio puente}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        % Paths, nodes and wires:
        \draw (16, 10) to[curved capacitor, l={$C1$}, label distance=0.02cm] (16, 8);
        \draw (16, 5) to[curved capacitor, l={$C2$}, label distance=0.02cm] (16, 3);
        \draw node[nigfete] (N1) at (20, 9) {} node
        [anchor=west] at (N1.text){$Q1$};
        \draw node[nigfete] (N2) at (20, 4.02) {} node
        [anchor=west] at (N2.text){$Q2$};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        % Paths, nodes and wires:
        \draw (4.25, 7.96) to[curved capacitor, l={$C1$}, label distance=0.02cm] (4.25, 5.96);
        \draw (4.25, 2.96) to[curved capacitor, l={$C2$}, label distance=0.02cm] (4.25, 0.96);
        \draw node[nigfete] (N1) at (8.25, 6.96) {} node 
        [anchor=west] at (N1.text){$Q1$};
        \draw node[nigfete] (N2) at (8.25, 1.98) {} node
        [anchor=west] at (N2.text){$Q2$};
