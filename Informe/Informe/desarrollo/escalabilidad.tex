...
%% Remarcar que el proyecto es escalable
%% Explicar qué se hizo para que el proyecto sea de esa forma
...
Para el diseño de la interfaz gráfica, se recomienda utilizar \textit{SquareLine Studio}, herramienta visual que facilita la creación de interfaces con la biblioteca LVGL. Para comenzar el proyecto, seleccionar la versión 9.1 de LVGL, y dentro de las opciones elegir ``Arduino'' y el proyecto de ejemplo ``\textit{Arduino with TFT\_eSPI}'', con las siguientes configuraciones:
\begin{itemize}
...
Una vez configurado, se pueden tomar como referencia las interfaces ya implementadas en los módulos existentes, ubicadas en ``ProyectoFinal/Prototipos/(\textit{módulo})/Interfaz de Usuario/Proyecto Interfaz''.
...
\begin{itemize}[noitemsep]
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item ui
            \begin{itemize}
                \item (archivos generados por SquareLine Studio)
            \end{itemize}
        \end{itemize} 
    \end{itemize}   
\end{itemize}

Esta estructura separa claramente la interfaz del usuario (carpeta \textit{ui}), la lógica de control del hardware (\textit{nombre\_control.c}), y la gestión de la iterfaz de usuario del módulo (\textit{nombre\_interface.c}).
...
Cada módulo requiere un archivo \textit{CMakeLists.txt} que define cómo ESP-IDF debe procesar el componente. Este archivo especifica los archivos fuente, directorios de inclusión, y dependencias del módulo. Se recomienda utilizar como plantilla los archivos \textit{CMakeLists.txt} de los módulos ya existentes.
...
Una vez implementadas estas funciones, debe declararse una la estructura \textit{module\_t} del módulo, que contiene su nombre descriptivo, el rango de voltaje ADC para identificación (en mV), y los punteros a las funciones de inicio y detención.
