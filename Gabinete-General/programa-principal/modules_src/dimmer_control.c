...
static void delete_adc();
#if USE_TEST_ZCD_SIGNAL
static void configure_test_signal();
#endif
static void start_adc_task();
static void stop_adc_task();
static uint16_t get_mapped_value(uint32_t x, uint16_t in_min, uint16_t in_max, uint16_t out_min, uint16_t out_max);
static void vTaskAdcRead(void *pvParameters);
...
static void delete_adc(){
    ...
    if(adc1_handle != NULL){
        ESP_ERROR_CHECK(adc_oneshot_del_unit(adc1_handle)); // Delete the ADC unit
        adc1_handle = NULL; // Reset the handle
    }
}
...
static void stop_adc_task(){
    // Stop the ADC read task
    if(xTaskAdcRead_handle != NULL){
        vTaskDelete(xTaskAdcRead_handle); // Delete the task
        xTaskAdcRead_handle = NULL; // Reset the task handle
    }
}
...
static void vTaskStopDimmerModule(void *pvParameters){
    if(isModuleRunning){
        ...
        stop_adc_task(); // Stop the ADC read task
        delete_timers(); // Stop the timers
        delete_adc(); // Delete the ADC handle
